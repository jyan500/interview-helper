/**
 * Profile picture card — the profile-photo upload, unchanged in behaviour from when it was the whole
 * Settings page; it's just been lifted into its own self-contained card so the page can compose it
 ...
 *
 * ONE FLOW FLAG (`saving`) spans the whole save/remove pipeline (upload + the API write), so the buttons
    ...
    return (
        <SettingsCard title="Profile picture">
            <div className="mt-5 flex items-center gap-6">
                {/* Avatar + a spinner overlay while saving. The overlay (and the pendingUrl bridge
                    behind it) is what masks the upload/refetch latency so the avatar doesn't blink
                    back to the default between "Save" and the image appearing. */}
